...
const Story = (props) => {
    ...
    const [likeColor, setLikeColor] = useState('#bdc3c7')
    ...
    const likeHandler = (e) => {
        ...
        if(likeColor === "#fd79a8"){   
            setLikeColor('#bdc3c7');
            axios.post('http://localhost:5000/stories/dislike/'+ID)
            .then(res => console.log(res))
            .catch(err => console.log(err));
        }
         //dislike to like   
            ...
            setLikeColor('#fd79a8');
            axios.post('http://localhost:5000/stories/like/'+ID)
            .then(res => console.log(res))
            .catch(err => console.log(err));
        }
    }
    ...
    return <div className={classes.story}>
        ...
        <button id="like" type="submit" value={props.id} className={classes.like} onClick={likeHandler}>
            <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" fill={likeColor} className="bi bi-star-fill" viewBox="0 0 16 16">
